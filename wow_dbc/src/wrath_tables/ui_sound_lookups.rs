...
pub type UISoundLookupsKey = crate::PrimaryKey<i32, UISoundLookups>;
...
impl UISoundLookups {
    pub const FILENAME: &'static str = "UISoundLookups.dbc";
    pub const FIELD_COUNT: usize = 3;
    pub const ROW_SIZE: usize = 12;

    pub fn verify(&self, sound_entries: &SoundEntries) -> Result<(), crate::InvalidForeignKeyError<&UISoundLookupsRow>> {
        for row in &self.rows {
            if row.sound_id.id != 0 && sound_entries.get(&row.sound_id).is_none() {
                let id = Some(row.id.id.into());
                return Err(crate::InvalidForeignKeyError::new(
                    std::any::type_name::<UISoundLookups>(),
                    row,
                    id,
...
impl Into<WrathTable> for UISoundLookups {
    fn into(self) -> WrathTable {
        WrathTable::UISoundLookups(self)
    }
}
...
impl DbcTable for UISoundLookups {
    ...
    fn rows(&self) -> &[UISoundLookupsRow] { &self.rows }
    fn rows_mut(&mut self) -> &mut [UISoundLookupsRow] { &mut self.rows }
...
#[allow(refining_impl_trait)]
impl Indexable<i32> for UISoundLookups {
    type Table = Self;

    fn get(&self, key: &UISoundLookupsKey) -> Option<&UISoundLookupsRow> {
        self.rows.iter().find(|a| &a.id == key)
    }

    fn get_mut(&mut self, key: &UISoundLookupsKey) -> Option<&mut UISoundLookupsRow> {
...
impl DbcRow for UISoundLookupsRow {
}

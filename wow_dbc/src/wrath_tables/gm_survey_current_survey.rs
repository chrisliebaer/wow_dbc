...
impl GMSurveyCurrentSurvey {
    ...
    pub fn verify(&self, gm_survey_surveys: &GMSurveySurveys) -> Result<(), crate::InvalidForeignKeyError<&GMSurveyCurrentSurveyRow>> {
        for row in &self.rows {
            if row.gm_survey_id.id != 0 && gm_survey_surveys.get(&row.gm_survey_id).is_none() {
                let id = Some(row.id.id.into());
                return Err(crate::InvalidForeignKeyError::new(
                    std::any::type_name::<GMSurveyCurrentSurvey>(),
                    row,
                    id,
                    row.gm_survey_id.id.into()
                ));
            }
...
impl Into<WrathTable> for GMSurveyCurrentSurvey {
    fn into(self) -> WrathTable {
        WrathTable::GMSurveyCurrentSurvey(self)
    }
}

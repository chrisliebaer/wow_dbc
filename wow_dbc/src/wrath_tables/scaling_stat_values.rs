...
pub type ScalingStatValuesKey = crate::PrimaryKey<i32, ScalingStatValues>;
...
impl ScalingStatValues {
    pub const FILENAME: &'static str = "ScalingStatValues.dbc";
...
impl DbcTable for ScalingStatValues {
    ...
    fn rows(&self) -> &[ScalingStatValuesRow] { &self.rows }
    fn rows_mut(&mut self) -> &mut [ScalingStatValuesRow] { &mut self.rows }
...
#[allow(refining_impl_trait)]
impl Indexable<i32> for ScalingStatValues {
    type Table = Self;

    fn get(&self, key: &ScalingStatValuesKey) -> Option<&ScalingStatValuesRow> {
        self.rows.iter().find(|a| &a.id == key)
    }

    fn get_mut(&mut self, key: &ScalingStatValuesKey) -> Option<&mut ScalingStatValuesRow> {
...
impl DbcRow for ScalingStatValuesRow {
}

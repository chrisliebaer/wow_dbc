use crate::{
    DbcRow, DbcTable, Indexable,
};
use crate::header::{
    DbcHeader, HEADER_SIZE, parse_header,
};
use crate::util::StringCache;
use crate::wrath_tables::item_purchase_group::{
    ItemPurchaseGroup, ItemPurchaseGroupKey,
};
use std::io::Write;
use super::WrathTable;

pub type ItemExtendedCostKey = crate::PrimaryKey<i32, ItemExtendedCost>;

#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
pub struct ItemExtendedCost {
    pub rows: Vec<ItemExtendedCostRow>,
}

impl ItemExtendedCost {
    pub const FILENAME: &'static str = "ItemExtendedCost.dbc";
    pub const FIELD_COUNT: usize = 16;
    pub const ROW_SIZE: usize = 64;

    pub fn verify(&self, item_purchase_group: &ItemPurchaseGroup) -> Result<(), crate::InvalidForeignKeyError<&ItemExtendedCostRow>> {
        for row in &self.rows {
            if row.item_purchase_group.id != 0 && item_purchase_group.get(&row.item_purchase_group).is_none() {
                let id = Some(row.id.id.into());
                return Err(crate::InvalidForeignKeyError::new(
                    std::any::type_name::<ItemExtendedCost>(),
                    row,
                    id,
                    row.item_purchase_group.id.into()
                ));
            }

        }

        Ok(())
    }

}

impl Into<WrathTable> for ItemExtendedCost {
    fn into(self) -> WrathTable {
        WrathTable::ItemExtendedCost(self)
    }
}

#[allow(refining_impl_trait)]
impl DbcTable for ItemExtendedCost {
    fn filename(&self) -> &'static str { Self::FILENAME }
    fn field_count(&self) -> usize { Self::FIELD_COUNT }
    fn row_size(&self) -> usize { Self::ROW_SIZE }

    fn rows(&self) -> &[ItemExtendedCostRow] { &self.rows }
    fn rows_mut(&mut self) -> &mut [ItemExtendedCostRow] { &mut self.rows }

    fn read(b: &mut impl std::io::Read) -> Result<Self, crate::DbcError> {
        let mut header = [0_u8; HEADER_SIZE];
        b.read_exact(&mut header)?;
        let header = parse_header(&header)?;

        if header.record_size != Self::ROW_SIZE as u32 {
            return Err(crate::DbcError::InvalidHeader(
                crate::InvalidHeaderError::RecordSize {
                    expected: Self::ROW_SIZE as u32,
                    actual: header.record_size,
                },
            ));
        }

        if header.field_count != Self::FIELD_COUNT as u32 {
            return Err(crate::DbcError::InvalidHeader(
                crate::InvalidHeaderError::FieldCount {
                    expected: Self::FIELD_COUNT as u32,
                    actual: header.field_count,
                },
            ));
        }

        let mut r = vec![0_u8; (header.record_count * header.record_size) as usize];
        b.read_exact(&mut r)?;

        let mut rows = Vec::with_capacity(header.record_count as usize);

        for mut chunk in r.chunks(header.record_size as usize) {
            let chunk = &mut chunk;

            // id: primary_key (ItemExtendedCost) int32
            let id = ItemExtendedCostKey::new(crate::util::read_i32_le(chunk)?);

            // honor_points: int32
            let honor_points = crate::util::read_i32_le(chunk)?;

            // arena_points: int32
            let arena_points = crate::util::read_i32_le(chunk)?;

            // arena_bracket: int32
            let arena_bracket = crate::util::read_i32_le(chunk)?;

            // item_id: int32[5]
            let item_id = crate::util::read_array_i32::<5>(chunk)?;

            // item_count: int32[5]
            let item_count = crate::util::read_array_i32::<5>(chunk)?;

            // required_arena_rating: int32
            let required_arena_rating = crate::util::read_i32_le(chunk)?;

            // item_purchase_group: foreign_key (ItemPurchaseGroup) int32
            let item_purchase_group = ItemPurchaseGroupKey::new(crate::util::read_i32_le(chunk)?.into());


            rows.push(ItemExtendedCostRow {
                id,
                honor_points,
                arena_points,
                arena_bracket,
                item_id,
                item_count,
                required_arena_rating,
                item_purchase_group,
            });
        }

        Ok(ItemExtendedCost { rows, })
    }

    fn write(&self, w: &mut impl Write) -> Result<(), std::io::Error> {
        let mut b = Vec::with_capacity(self.rows.len() * Self::ROW_SIZE);

        let  string_cache = StringCache::new();

        for row in &self.rows {
            // id: primary_key (ItemExtendedCost) int32
            b.write_all(&row.id.id.to_le_bytes())?;

            // honor_points: int32
            b.write_all(&row.honor_points.to_le_bytes())?;

            // arena_points: int32
            b.write_all(&row.arena_points.to_le_bytes())?;

            // arena_bracket: int32
            b.write_all(&row.arena_bracket.to_le_bytes())?;

            // item_id: int32[5]
            for i in row.item_id {
                b.write_all(&i.to_le_bytes())?;
            }


            // item_count: int32[5]
            for i in row.item_count {
                b.write_all(&i.to_le_bytes())?;
            }


            // required_arena_rating: int32
            b.write_all(&row.required_arena_rating.to_le_bytes())?;

            // item_purchase_group: foreign_key (ItemPurchaseGroup) int32
            b.write_all(&(row.item_purchase_group.id as i32).to_le_bytes())?;

        }

        assert_eq!(b.len(), self.rows.len() * Self::ROW_SIZE);
        let header = DbcHeader {
            record_count: self.rows.len() as u32,
            field_count: Self::FIELD_COUNT as u32,
            record_size: Self::ROW_SIZE as u32,
            string_block_size: string_cache.size(),
        };

        w.write_all(&header.write_header())?;
        w.write_all(&b)?;
        w.write_all(string_cache.buffer())?;
        Ok(())
    }

}

#[allow(refining_impl_trait)]
impl Indexable<i32> for ItemExtendedCost {
    type Table = Self;

    fn get(&self, key: &ItemExtendedCostKey) -> Option<&ItemExtendedCostRow> {
        self.rows.iter().find(|a| &a.id == key)
    }

    fn get_mut(&mut self, key: &ItemExtendedCostKey) -> Option<&mut ItemExtendedCostRow> {
        self.rows.iter_mut().find(|a| &a.id == key)
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
pub struct ItemExtendedCostRow {
    pub id: ItemExtendedCostKey,
    pub honor_points: i32,
    pub arena_points: i32,
    pub arena_bracket: i32,
    pub item_id: [i32; 5],
    pub item_count: [i32; 5],
    pub required_arena_rating: i32,
    pub item_purchase_group: ItemPurchaseGroupKey,
}

impl DbcRow for ItemExtendedCostRow {
}

#[cfg(test)]
mod test {
    use super::*;
    use std::fs::File;
    use std::io::Read;

    #[test]
    #[ignore = "requires DBC files"]
    fn item_extended_cost() {
        let mut file = File::open("../wrath-dbc/ItemExtendedCost.dbc").expect("Failed to open DBC file");
        let mut contents = Vec::new();
        file.read_to_end(&mut contents).expect("Failed to read DBC file");
        let actual = ItemExtendedCost::read(&mut contents.as_slice()).unwrap();
        let mut v = Vec::with_capacity(contents.len());
        actual.write(&mut v).unwrap();
        let new = ItemExtendedCost::read(&mut v.as_slice()).unwrap();
        assert_eq!(actual, new);
    }
}

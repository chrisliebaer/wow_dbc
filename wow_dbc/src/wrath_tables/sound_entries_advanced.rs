...
pub type SoundEntriesAdvancedKey = crate::PrimaryKey<i32, SoundEntriesAdvanced>;
...
impl SoundEntriesAdvanced {
    pub const FILENAME: &'static str = "SoundEntriesAdvanced.dbc";
    pub const FIELD_COUNT: usize = 24;
    pub const ROW_SIZE: usize = 96;

    pub fn verify(&self, sound_entries: &SoundEntries) -> Result<(), crate::InvalidForeignKeyError<&SoundEntriesAdvancedRow>> {
        for row in &self.rows {
            if row.sound_entry_id.id != 0 && sound_entries.get(&row.sound_entry_id).is_none() {
                let id = Some(row.id.id.into());
                return Err(crate::InvalidForeignKeyError::new(
                    std::any::type_name::<SoundEntriesAdvanced>(),
                    row,
                    id,
                    row.sound_entry_id.id.into()
                ));
            }
...
impl Into<WrathTable> for SoundEntriesAdvanced {
    fn into(self) -> WrathTable {
        WrathTable::SoundEntriesAdvanced(self)
    }
}
...
impl DbcTable for SoundEntriesAdvanced {
    ...
    fn rows(&self) -> &[SoundEntriesAdvancedRow] { &self.rows }
    fn rows_mut(&mut self) -> &mut [SoundEntriesAdvancedRow] { &mut self.rows }
...
#[allow(refining_impl_trait)]
impl Indexable<i32> for SoundEntriesAdvanced {
    type Table = Self;

    fn get(&self, key: &SoundEntriesAdvancedKey) -> Option<&SoundEntriesAdvancedRow> {
        self.rows.iter().find(|a| &a.id == key)
    }

    fn get_mut(&mut self, key: &SoundEntriesAdvancedKey) -> Option<&mut SoundEntriesAdvancedRow> {
...
impl DbcRow for SoundEntriesAdvancedRow {
}

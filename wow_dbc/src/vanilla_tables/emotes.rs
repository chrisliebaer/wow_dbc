...
pub type EmotesKey = crate::PrimaryKey<u32, Emotes>;
...
impl Emotes {
    ...
    pub fn verify(&self, animation_data: &AnimationData, sound_entries: &SoundEntries) -> Result<(), crate::InvalidForeignKeyError<&EmotesRow>> {
        for row in &self.rows {
            ...
            if row.event_sound_entry.id != 0 && sound_entries.get(&row.event_sound_entry).is_none() {
                let id = Some(row.id.id.into());
                return Err(crate::InvalidForeignKeyError::new(
                    std::any::type_name::<Emotes>(),
                    row,
                    id,
                    row.event_sound_entry.id.into()
                ));
            }

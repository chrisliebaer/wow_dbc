...
pub type ZoneIntroMusicTableKey = crate::PrimaryKey<u32, ZoneIntroMusicTable>;
...
impl ZoneIntroMusicTable {
    ...
    pub fn verify(&self, sound_entries: &SoundEntries) -> Result<(), crate::InvalidForeignKeyError<&ZoneIntroMusicTableRow>> {
        for row in &self.rows {
            if row.intro_sound.id != 0 && sound_entries.get(&row.intro_sound).is_none() {
...
impl Into<VanillaTable> for ZoneIntroMusicTable {
    fn into(self) -> VanillaTable {
        VanillaTable::ZoneIntroMusicTable(self)
    }
}
...
#[allow(refining_impl_trait)]
impl Indexable<u32> for ZoneIntroMusicTable {
    type Table = Self;

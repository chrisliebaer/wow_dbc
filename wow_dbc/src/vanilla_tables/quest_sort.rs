...
pub type QuestSortKey = crate::PrimaryKey<u32, QuestSort>;
...
impl QuestSort {
    pub const FILENAME: &'static str = "QuestSort.dbc";
    pub const FIELD_COUNT: usize = 10;
    pub const ROW_SIZE: usize = 40;

}

impl Into<VanillaTable> for QuestSort {
    fn into(self) -> VanillaTable {
        VanillaTable::QuestSort(self)
    }
}
...
impl DbcTable for QuestSort {
    ...
    fn rows(&self) -> &[QuestSortRow] { &self.rows }
    fn rows_mut(&mut self) -> &mut [QuestSortRow] { &mut self.rows }
...
#[allow(refining_impl_trait)]
impl Indexable<u32> for QuestSort {
    type Table = Self;

    fn get(&self, key: &QuestSortKey) -> Option<&QuestSortRow> {
        self.rows.iter().find(|a| &a.id == key)
    }

    fn get_mut(&mut self, key: &QuestSortKey) -> Option<&mut QuestSortRow> {

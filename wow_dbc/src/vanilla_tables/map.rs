...
pub type MapKey = crate::PrimaryKey<u32, Map>;
...
impl Map {
    ...
    pub fn verify(&self, area_table: &AreaTable, loading_screens: &LoadingScreens) -> Result<(), crate::InvalidForeignKeyError<&MapRow>> {
        for row in &self.rows {
            ...
            if row.loading_screen.id != 0 && loading_screens.get(&row.loading_screen).is_none() {
                let id = Some(row.id.id.into());
                return Err(crate::InvalidForeignKeyError::new(
                    std::any::type_name::<Map>(),
                    row,
                    id,
                    row.loading_screen.id.into()
                ));
            }

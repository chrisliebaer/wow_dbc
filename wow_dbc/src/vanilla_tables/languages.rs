...
pub type LanguagesKey = crate::PrimaryKey<u32, Languages>;
...
impl Languages {
    pub const FILENAME: &'static str = "Languages.dbc";
...
impl DbcTable for Languages {
    ...
    fn rows(&self) -> &[LanguagesRow] { &self.rows }
    fn rows_mut(&mut self) -> &mut [LanguagesRow] { &mut self.rows }
...
#[allow(refining_impl_trait)]
impl Indexable<u32> for Languages {
    type Table = Self;

    fn get(&self, key: &LanguagesKey) -> Option<&LanguagesRow> {
        self.rows.iter().find(|a| &a.id == key)
    }

    fn get_mut(&mut self, key: &LanguagesKey) -> Option<&mut LanguagesRow> {

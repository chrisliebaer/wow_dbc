...
pub type SpellAuraNamesKey = crate::PrimaryKey<u32, SpellAuraNames>;
...
impl SpellAuraNames {
    pub const FILENAME: &'static str = "SpellAuraNames.dbc";
    pub const FIELD_COUNT: usize = 12;
    pub const ROW_SIZE: usize = 48;

}

impl Into<VanillaTable> for SpellAuraNames {
    fn into(self) -> VanillaTable {
        VanillaTable::SpellAuraNames(self)
    }
}
...
impl DbcTable for SpellAuraNames {
    ...
    fn rows(&self) -> &[SpellAuraNamesRow] { &self.rows }
    fn rows_mut(&mut self) -> &mut [SpellAuraNamesRow] { &mut self.rows }
...
#[allow(refining_impl_trait)]
impl Indexable<u32> for SpellAuraNames {
    type Table = Self;

    fn get(&self, key: &SpellAuraNamesKey) -> Option<&SpellAuraNamesRow> {
        self.rows.iter().find(|a| &a.id == key)
    }

    fn get_mut(&mut self, key: &SpellAuraNamesKey) -> Option<&mut SpellAuraNamesRow> {
...
impl DbcRow for SpellAuraNamesRow {
}

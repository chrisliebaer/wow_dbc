...
pub type ItemClassKey = crate::PrimaryKey<u32, ItemClass>;
...
impl ItemClass {
    pub const FILENAME: &'static str = "ItemClass.dbc";
...
impl DbcTable for ItemClass {
    ...
    fn rows(&self) -> &[ItemClassRow] { &self.rows }
    fn rows_mut(&mut self) -> &mut [ItemClassRow] { &mut self.rows }
...
#[allow(refining_impl_trait)]
impl Indexable<u32> for ItemClass {
    type Table = Self;

    fn get(&self, key: &ItemClassKey) -> Option<&ItemClassRow> {
        self.rows.iter().find(|a| &a.id == key)
    }

    fn get_mut(&mut self, key: &ItemClassKey) -> Option<&mut ItemClassRow> {

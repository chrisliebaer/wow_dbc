...
pub type UnitBloodKey = crate::PrimaryKey<u32, UnitBlood>;
...
impl UnitBlood {
    pub const FILENAME: &'static str = "UnitBlood.dbc";
    pub const FIELD_COUNT: usize = 10;
    pub const ROW_SIZE: usize = 40;

}

impl Into<VanillaTable> for UnitBlood {
    fn into(self) -> VanillaTable {
        VanillaTable::UnitBlood(self)
    }
}
...
impl DbcTable for UnitBlood {
    ...
    fn rows(&self) -> &[UnitBloodRow] { &self.rows }
    fn rows_mut(&mut self) -> &mut [UnitBloodRow] { &mut self.rows }
...
#[allow(refining_impl_trait)]
impl Indexable<u32> for UnitBlood {
    type Table = Self;

    fn get(&self, key: &UnitBloodKey) -> Option<&UnitBloodRow> {
        self.rows.iter().find(|a| &a.id == key)
    }

    fn get_mut(&mut self, key: &UnitBloodKey) -> Option<&mut UnitBloodRow> {

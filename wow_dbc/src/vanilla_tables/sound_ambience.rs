...
pub type SoundAmbienceKey = crate::PrimaryKey<u32, SoundAmbience>;
...
impl SoundAmbience {
    pub const FILENAME: &'static str = "SoundAmbience.dbc";
    pub const FIELD_COUNT: usize = 3;
    pub const ROW_SIZE: usize = 12;

    pub fn verify(&self, sound_entries: &SoundEntries) -> Result<(), crate::InvalidForeignKeyError<&SoundAmbienceRow>> {
        for row in &self.rows {
            if row.day_sound.id != 0 && sound_entries.get(&row.day_sound).is_none() {
                let id = Some(row.id.id.into());
                return Err(crate::InvalidForeignKeyError::new(
                    std::any::type_name::<SoundAmbience>(),
                    row,
                    id,
...
impl Into<VanillaTable> for SoundAmbience {
    fn into(self) -> VanillaTable {
        VanillaTable::SoundAmbience(self)
    }
}
...
impl DbcTable for SoundAmbience {
    ...
    fn rows(&self) -> &[SoundAmbienceRow] { &self.rows }
    fn rows_mut(&mut self) -> &mut [SoundAmbienceRow] { &mut self.rows }
...
#[allow(refining_impl_trait)]
impl Indexable<u32> for SoundAmbience {
    type Table = Self;

    fn get(&self, key: &SoundAmbienceKey) -> Option<&SoundAmbienceRow> {
        self.rows.iter().find(|a| &a.id == key)
    }

    fn get_mut(&mut self, key: &SoundAmbienceKey) -> Option<&mut SoundAmbienceRow> {
...
impl DbcRow for SoundAmbienceRow {
}

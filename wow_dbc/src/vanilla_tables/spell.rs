...
use crate::vanilla_tables::spell_range::{
    SpellRange, SpellRangeKey,
};
use crate::vanilla_tables::spell_shapeshift_form::{
    SpellShapeshiftForm, SpellShapeshiftFormKey,
};
use std::io::Write;
...
pub type SpellKey = crate::PrimaryKey<u32, Spell>;
...
impl Spell {
    ...
    pub const FIELD_COUNT: usize = 173;
    pub const ROW_SIZE: usize = 692;

    pub fn verify(&self, chr_classes: &ChrClasses, creature_type: &CreatureType, item_class: &ItemClass, resistances: &Resistances, spell_cast_times: &SpellCastTimes, spell_category: &SpellCategory, spell_dispel_type: &SpellDispelType, spell_duration: &SpellDuration, spell_focus_object: &SpellFocusObject, spell_icon: &SpellIcon, spell_mechanic: &SpellMechanic, spell_range: &SpellRange, spell_shapeshift_form: &SpellShapeshiftForm) -> Result<(), crate::InvalidForeignKeyError<&SpellRow>> {
        for row in &self.rows {
            if row.school.id != 0 && resistances.get(&row.school).is_none() {
            ...
            if row.shapeshift_mask.id != 0 && spell_shapeshift_form.get(&row.shapeshift_mask).is_none() {
                let id = Some(row.id.id.into());
                return Err(crate::InvalidForeignKeyError::new(
                    std::any::type_name::<Spell>(),
                    row,
                    id,
                    row.shapeshift_mask.id.into()
                ));
            }

            if row.shapeshift_exclude.id != 0 && spell_shapeshift_form.get(&row.shapeshift_exclude).is_none() {
                let id = Some(row.id.id.into());
                return Err(crate::InvalidForeignKeyError::new(
                    std::any::type_name::<Spell>(),
                    row,
                    id,
                    row.shapeshift_exclude.id.into()
                ));
            }

            if row.target_creature_type.id != 0 && creature_type.get(&row.target_creature_type).is_none() {
                let id = Some(row.id.id.into());
                return Err(crate::InvalidForeignKeyError::new(
                    std::any::type_name::<Spell>(),
                    row,
                    id,
                    row.target_creature_type.id.into()
                ));
            }
            ...
            if row.duration.id != 0 && spell_duration.get(&row.duration).is_none() {
                let id = Some(row.id.id.into());
                return Err(crate::InvalidForeignKeyError::new(
                    std::any::type_name::<Spell>(),
                    row,
                    id,
                    row.duration.id.into()
                ));
            }

            if row.range.id != 0 && spell_range.get(&row.range).is_none() {
                let id = Some(row.id.id.into());
                return Err(crate::InvalidForeignKeyError::new(
                    std::any::type_name::<Spell>(),
                    row,
            ...
            if row.modal_next_spell.id != 0 && self.get(&row.modal_next_spell).is_none() {
                let id = Some(row.id.id.into());
                return Err(crate::InvalidForeignKeyError::new(
                    std::any::type_name::<Spell>(),
                    row,
                    id,
                    row.modal_next_spell.id.into()
                ));
            }

            if row.equipped_item_class.id != 0 && item_class.get(&row.equipped_item_class).is_none() {
                let id = Some(row.id.id.into());
                return Err(crate::InvalidForeignKeyError::new(
                    std::any::type_name::<Spell>(),
                    row,
                    id,
                    row.equipped_item_class.id.into()
                ));
            }
            ...
            if row.spell_class_set.id != 0 && chr_classes.get(&row.spell_class_set).is_none() {
                let id = Some(row.id.id.into());
                return Err(crate::InvalidForeignKeyError::new(
                    std::any::type_name::<Spell>(),
                    row,
                    id,
                    row.spell_class_set.id.into()
                ));
            }
